...
import type { PostWithRelations, PostFormData } from '~/types'
...
interface PostFormProps {
  post?: PostWithRelations & { category_ids: string[]; tag_ids: string[] }
}
...
export function PostForm({ post }: PostFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      tags: selectedTags.map((t) => ({ id: t.id, name: t.name, slug: t.slug })),
    ...
    try {
      ...
      setTimeout(() => navigate({ to: '/admin/posts' }), 600)
    } catch (err) {
      setToast({
        open: true,
            ...
            <Card variant="outlined">
              <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                ...
                <Box>
                  ...
                  <Typography variant="body2">{new Date(post.created_at).toLocaleString()}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={500} display="block">Updated:</Typography>
                  <Typography variant="body2">{new Date(post.updated_at).toLocaleString()}</Typography>
                </Box>
                {post.published_at && (
                  <Box>
                    <Typography variant="caption" color="text.secondary" fontWeight={500} display="block">Published:</Typography>
                    <Typography variant="body2">{new Date(post.published_at).toLocaleString()}</Typography>
                  </Box>
                )}
              </CardContent>
            </Card>
