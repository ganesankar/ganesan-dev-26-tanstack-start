...
export function PageForm({ page }: PageFormProps) {
  ...
  function handleTitleChange(title: string) {
    setForm((prev) => ({
      ...prev,
      title,
      slug: page ? prev.slug : slugify(title),
    }))
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      ...(form.published && !page?.published_at
    ...
    try {
      if (page) {
        ...
        setToast({ open: true, message: 'Page updated', severity: 'success' })
      } else {
        ...
        setToast({ open: true, message: 'Page created', severity: 'success' })
      }
      await queryClient.invalidateQueries({ queryKey: pageKeys.all })
      setTimeout(() => navigate({ to: '/admin/pages' }), 600)
    } catch (err) {
      setToast({
        open: true,
            ...
            <Card variant="outlined">
              <CardContent sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                ...
                <Box>
                  ...
                  <Typography variant="body2">{new Date(page.created_at).toLocaleString()}</Typography>
                </Box>
                <Box>
                  <Typography variant="caption" color="text.secondary" fontWeight={500} display="block">Updated:</Typography>
                  <Typography variant="body2">{new Date(page.updated_at).toLocaleString()}</Typography>
                </Box>
                {page.published_at && (
                  <Box>
                    <Typography variant="caption" color="text.secondary" fontWeight={500} display="block">Published:</Typography>
                    <Typography variant="body2">{new Date(page.published_at).toLocaleString()}</Typography>
                  </Box>
                )}
              </CardContent>
            </Card>
